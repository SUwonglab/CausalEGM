...
class CausalEGM(object):
    ...
    def evaluate(self, data, nb_intervals=200):
        ...
        data_v_ = self.g_net.predict(data_z,batch_size=32)
        data_z_ = self.e_net.predict(data_v,batch_size=32)
        #data_v_ = self.g_net(data_z)
        #data_z_ = self.e_net(data_v)
        data_z0 = data_z_[:,:self.params['z_dims'][0]]
        data_z1 = data_z_[:,self.params['z_dims'][0]:sum(self.params['z_dims'][:2])]
        data_z2 = data_z_[:,sum(self.params['z_dims'][:2]):sum(self.params['z_dims'][:3])]
        data_z3 = data_z_[:-self.params['z_dims'][3]:]
        data_y_pred = self.f_net.predict(tf.concat([data_z0, data_z2, data_x], axis=-1),batch_size=32)
        data_x_pred = self.h_net.predict(tf.concat([data_z0, data_z1], axis=-1),batch_size=32)
        ...
        if self.params['binary_treatment']:
            ...
            y_pred_pos = self.f_net.predict(tf.concat([data_z0, data_z2, np.ones((len(data_x),1))], axis=-1),batch_size=32)
            y_pred_neg = self.f_net.predict(tf.concat([data_z0, data_z2, np.zeros((len(data_x),1))], axis=-1),batch_size=32)
            ...
            for x in np.linspace(self.params['x_min'], self.params['x_max'], nb_intervals):
                ...
                y_pred = self.f_net.predict(tf.concat([data_z0, data_z2, data_x], axis=-1),batch_size=32)
