...
class scRNA_Sox2FGF4_Data(object):
    def __init__(self, N = 5877, v_dim=599):
        self.data = np.load('Sox2FGF4Data.npy')
        self.sample_size = N
        self.v_dim = v_dim
        self.data_v = self.data[:, 2:].astype('float32')
        ## (N,1)
        self.data_x = self.data[:, 0].reshape(-1, 1).astype('float32')
        self.data_y = (self.data[:, 1]).reshape(-1, 1).astype('float32')
...
class linear_effect_non_linear(object):
    ...
    def get_value_y(self, v, x):
        ...
        np.dot(v[:,(self.z0_dim+self.z1_dim):(self.z0_dim+self.z1_dim+self.z2_dim)], self.beta_2)) , 0.5)

    def load_all(self):
        return self.data_x, self.data_y, self.data_v


class quadratic_effect(object):
...
class Imbens_Sim(object):
    def __init__(self, N = 20000, v_dim=12, z0_dim=1, z1_dim=1, z2_dim=1, z3_dim=9, ax = 1, bx = 1):
        self.data = np.load('Imbens_simulated_data_N20000_vdim12.npy')
        self.sample_size = N
        self.v_dim = v_dim
...
class Sun_Sim(object):
    def __init__(self, N = 20000, v_dim=200, z0_dim=2, z1_dim=2, z2_dim=2, z3_dim=4):
        np.random.seed(123)
        self.sample_size = N
        self.v_dim = v_dim
        self.z0_dim = z0_dim
    ...
    #get x given v(or z1)
    def get_value_x(self, v):
        return np.random.normal(self.f_1(v[:,0])+ self.f_2(v[:,1])+ self.f_3(v[:,2]) + self.f_4(v[:,3]), 1).reshape(-1, 1)

    #get y given x and v(or z1)
    def get_value_y(self, v, x):
        return np.random.normal((self.f_3(v[:,0])+ self.f_4(v[:,1]) + self.f_5(v[:,4])+ self.f_6(v[:,5])).reshape(-1, 1) +  \
            x, 1).reshape(-1, 1)
